...
export const EventItem: FC<EventItemProps> = memo(({ event, position, getDatePosition }) => {
  ...
  const iconEmoji = getIcon(event.icon);
  ...
  return (
    ...
    >
      ...
      {/* Event card */}
      <div className={`lifecycle-event-card lifecycle-event-card-${event.type}`}>
        {/* Content */}
        <div className="lifecycle-event-card-content">
          ...
          {event.showParticipantNames && event.participants && event.participants.length > 0 && (
